...
<div class="map_wrap">
    ...
    <div id="menu_wrap" class="bg_white">
        <div class="option">
          <div class="row mb-3">
              <div class="col">
                <select class="selectBox" name="sale" v-model="selected" @change="showWhat()" >
                  <option value=1>판매 중 매물</option>
                  <option value=0>판매 가능 매물</option>
                </select>
  ...
  methods: {
    ...
    async searchPlaces() {
      ...
      if (!keyword.replace(/^\s+|\s+$/g, '')) {
        if(this.selected==1){
          await this.$store.dispatch('sellingHouseListMap','');
          this.makingMarker();
        }
        else{
          await this.$store.dispatch('emptyHouseListMap','');
          this.makingMarker();
        }
      }
      else{
        if(this.selected==1){
          await this.$store.dispatch('sellingHouseListMap',keyword);
          this.makingMarker();
        }
        else{
          await this.$store.dispatch('emptyHouseListMap',keyword);
          this.makingMarker();
        }
    ...
    for ( var i=0; i<places.length; i++ ) {
        ...
        (function(marker, place, placePosition, index, detail) {
            ...
            if(detail.selected==1) content += '<span class="markerSpan">판매자 : '+ place.seller +'</span><br>'
            if(detail.selected==1) content += '<p class="markerSpan">가격 : ' + place.dealAmount +' 만원</p>'
            content += '<p class="markerSpan">평수 : ' + place.area +'m<sup>2</sup></p>'
            if(detail.selected==1) content += '<a id="detail1" class="markerBrn" style="color:#fff">상세 페이지</a>'
            else content += '<a id="detail1" class="markerBrn" style="color:#fff">판매 등록 하기</a>'
            content += '</div></div></div>'
            infowindow.setContent(content);
            infowindow.open(map, marker);

            document.getElementById("detail1").onclick = function(){
              if(detail.selected==1) detail.toDetail();
              else detail.addProperty(place);
            }
            });
            ...
            if(detail.selected==1) content += '<p class="markerSpan">가격 : ' + place.dealAmount +' 만원</p>'
            content += '<p class="markerSpan">평수 : ' + place.area +'m<sup>2</sup></p>'
            if(detail.selected==1) content += '<a id="detail" class="markerBrn" style="color:#fff">상세 페이지</a>'
            else content += '<a id="detail" class="markerBrn" style="color:#fff">판매 등록 하기</a>'
            content += '</div></div></div>'
            infowindow.setContent(content);
            infowindow.open(map, marker);
    ...
    async addProperty(deal){
      
      this.$store.state.sellerDetail = deal;
      console.log(deal);
      alert('해당 매물의 수정창으로 이동합니다!');
      this.$router.push("/PropertyRegist");
    },
