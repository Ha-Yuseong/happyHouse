      ...
      <nav class="pagination-a">
        <ul class="pagination justify-content-center">
          ...
          <li v-for="index in ( endPageIndex-startPageIndex + 1 )" :key="index"
              v-bind:class="{active: (startPageIndex + index - 1 == $store.state.houseInfo.currentPageIndex)}" class="page-item">
...
export default {
  ...
  computed: {
    pageCount: function(){
      return this.$store.getters.getHousePageCount;
    },
    startPageIndex: function(){
      return this.$store.getters.getHouseStartPageIndex;
    },
    endPageIndex: function(){
      return this.$store.getters.getHouseEndPageIndex;
    },
    prev: function(){
      return this.$store.getters.getHousePrev;
    },
    next: function(){
      return this.$store.getters.getHouseNext;
    }
  },
  methods:{
    // 부모에게 event 전달    
