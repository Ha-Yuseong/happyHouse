        ...
        <div class="option">
          <div class="row mb-3">
              <div class="col">
                ...
                <select class="selectBox" name="dong">
                  <option v-for="(dong,index) in $store.getters.getDongList" :key="index" :value="dong.code">{{dong.name}}</option>
                </select>
              </div>
            </div>
            <div>
...
export default {
  ...
  mounted() {
    if (window.kakao && window.kakao.maps) {
      ...
      script.src =
        "http://dapi.kakao.com/v2/maps/sdk.js?autoload=false&appkey=38c01a7595acefb3baa681531cd36c4c&libraries=services,clusterer,drawing";
  ...
  methods: {
    ...
    searchPlaces() {
      ...
      if(this.$store.state.houseInfo.list.length==0){
        alert('해당 지역엔 판매중인 매물이 없습니다.');
      }
      else{
    ...
    console.log('총 갯수 : ' + places.length);
    ...
    for ( var i=0; i<places.length; i++ ) {
        ...
        (function(marker, place, placePosition, index, detail) {
            ...
            detail.houseDetail = place;
            detail.currImage = place.img+( (index % 9)+1) + '.jpg';
            ...
            content += '<div class="row"> <div class="col"> <div class="markerBoxImg" style="background:url(' + place.img + ( (index % 9)+1) + '.jpg) no-repeat; background-size: cover;"/>'
            content += '</div> <div class="col"><span class="markerSpan">서울특별시 ' +  place.gu  + ' ' +place.dong + ' ' + place.jibun + '</span></div></div>'
            content += '<div class="col"><button class="btn btn-info">연락하기</button></div>'
            content += '</div>'
            ...
            content += '<span class="markerSpan">판매자 : 누구누구</span><br>'
            content += '<p class="markerSpan">가격 :' + place.dealAmount +'만원</p>'
            content += '<p class="markerSpan">평수 :' + place.area +'m<sup>2</sup></p>'
            content += '<a href="propertyDetail" class="markerBrn float-end" style="color:#fff">상세 페이지</a>'
            content += '</div></div></div>'
            infowindow.setContent(content);
            infowindow.open(map, marker);
            };
        })(marker, places[i], placePosition, i, this.$store.state);
    ...
    getDong(event){
      this.$store.dispatch('dongList',event.target.value);
    }
  ...
  created() {
      ...
      this.$store.dispatch('dongList',11110);
